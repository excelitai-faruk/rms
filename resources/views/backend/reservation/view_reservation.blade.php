                    ...
                    <div class="card-body">
                        ...
                        <div id="datatable-buttons_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                                ...
                                <div class="col-sm-12 col-md-6">
                                    <div class="dt-buttons btn-group flex-wrap">
                                        ...
                                        <div class="col-sm-12 col-md-6"><div id="datatable-buttons_filter" class="dataTables_filter">
                                            ...
                                            <a href="{{ route('reservation.add') }}" class="btn btn-success">Add Reservation</a> 
                            ...
                            <thead>
                                ...
                                aria-label="Address: activate to sort column ascending">Customer_Name</th>

                                <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 0px;"
                                 aria-label="Contact_Person: activate to sort column ascending">Table No</th>

                                 <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 0px;"
                                  aria-label="Phone: activate to sort column ascending">Person Number</th>

                                  <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 0px;"
                                aria-label="Address: activate to sort column ascending">Start Time</th>

                                <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 0px;"
                                aria-label="Address: activate to sort column ascending">End Time</th>

                                <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 0px;"
                                 aria-label="Contact_Person: activate to sort column ascending">Date</th>
                                 
                                 <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 0px;"
                                  aria-label="Phone: activate to sort column ascending">Status</th>
...
{{--                                 
                                ...
                                $table->string('status');
                                 --}}
                              @foreach ($reservation_view as $r_view )
                                        <tr>
                                          
                                          <td>{{$r_view->si}}</td>
                                          <td>{{$r_view->customer_name}}</td>
                                          <td>{{$r_view->table_no}}</td>
                                          <td>{{$r_view->person_number}}</td>
                                          <td>{{$r_view->start_time}}</td>
                                          <td>{{$r_view->end_time}}</td>
                                          <td>{{$r_view->date}}</td>
                                          <td>{{$r_view->status}}</td>
                                    
                                   
                                            <td style="display:flex"> 
                                             <a style="margin: 0px 6px" href="{{ route('reservation.edit', $r_view->id ) }}" class="btn btn-success">Edit</a> 

                                             <form action="{{ route('reservation.delete', $r_view->id ) }}" method="POST">
                                                @csrf
                                                <input type="hidden" name="p_id" id="p_id" value="{{$r_view->id}}">
